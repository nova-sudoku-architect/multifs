...
<D:propertyupdate xmlns:D="DAV:">
  <D:set>
    ...
    #[test]
    fn test_webdav_browser_get_directory_listing() {
        // Simulate what handle_get does for browser GET requests
        // For path "bucket/subdir/":
        let path = "bucket/subdir/";
        let trimmed = path.trim_end_matches('/');
        assert_eq!(trimmed, "bucket/subdir");
        ...
        assert_eq!(key, "subdir");

        // After trimming, key is NOT empty and does NOT end with /, so the
        // OLD code would NOT detect this as a directory and would fail.
        // The fix: check the ORIGINAL path (before trim) for trailing slash.
        let is_directory = key.is_empty() || path.ends_with('/');
        assert!(is_directory, "Should detect as directory: original path ends with /");

        // For "bucket/file.txt" (no trailing slash) it should NOT be a directory
        let path2 = "bucket/file.txt";
        let trimmed2 = path2.trim_end_matches('/');
        let parts2: Vec<&str> = trimmed2.splitn(2, '/').collect();
        let key2 = parts2.get(1).copied().unwrap_or("");
        let is_directory2 = key2.is_empty() || path2.ends_with('/');
        assert!(!is_directory2, "Should not detect file as directory");

        // For "bucket/" (root of a bucket) it should be a directory
        let path3 = "bucket/";
        let trimmed3 = path3.trim_end_matches('/');
        let parts3: Vec<&str> = trimmed3.splitn(2, '/').collect();
        let key3 = parts3.get(1).copied().unwrap_or("");
        let is_directory3 = key3.is_empty() || path3.ends_with('/');
        assert!(is_directory3, "Should detect bucket root as directory");
    }

    #[test]
    fn test_webdav_prefix_extraction() {
        // When navigating to a subdirectory like bucket/subdir/
        // the prefix should be "subdir/" so list_objects filters correctly.

        let test_cases = vec![
            ("video/BLOR-074/", "video", "BLOR-074", Some("BLOR-074/")),
            ("video/", "video", "", None),
            ("home/files/", "home", "files", Some("files/")),
            ("bucket/file.txt", "bucket", "file.txt", None), // real file, not directory
        ];

        for (input_path, expected_bucket, expected_key, expected_prefix) in test_cases {
            let trimmed = input_path.trim_end_matches('/');
            let parts: Vec<&str> = trimmed.splitn(2, '/').collect();
            let bucket = parts[0];
            let key = parts.get(1).copied().unwrap_or("");

            assert_eq!(bucket, expected_bucket, "Path {}: bucket mismatch", input_path);
            assert_eq!(key, expected_key, "Path {}: key mismatch", input_path);

            // Extract prefix if this is a directory request
            let is_dir = key.is_empty() || input_path.ends_with('/');
            let prefix = if is_dir && !key.is_empty() {
                Some(key.to_string() + "/")
            } else {
                None
            };
            assert_eq!(prefix.as_deref(), expected_prefix, "Path {}: prefix mismatch", input_path);
        }
    }
